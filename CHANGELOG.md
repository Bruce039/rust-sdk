...
* [BREAKING][type][rust,rpc] Adopted the canonical `miden-objects` Protobuf messages. The node schemas now import them, so the client generates only the node-owned `rpc` and `submission` messages and re-exports the canonical packages from `miden-objects`. The hand-written conversions in `rpc::domain` are gone in favor of `miden_objects::conversion`, which removes the `rpc::domain::digest`, `rpc::domain::merkle`, `rpc::domain::smt`, and `rpc::domain::block` modules. `primitives::Digest` is now `primitives::Word`, `Asset` moved to the `asset` package, and the transaction encryption messages moved to the `submission` package. Requires a node built from [#2621](https://github.com/0xMiden/node/pull/2621) or later.
